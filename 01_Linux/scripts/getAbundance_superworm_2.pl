...
$output = 'superworm_MAG_abundances_reads.txt';
system("head -1 superworm_coverage.txt | sed 's/\#rname/MAG_name/' | sed 's/\tendpos//' | sed 's/Superworm_MAGs_combined_//g' > superworm_MAG_abundances_reads.txt");
open($in, '<', $MAG_names);
while(<$in>) {
  chomp;
  $MAG = $_;
  @total_coverage = (0) x 24;
  @total_length = (0) x 24;
