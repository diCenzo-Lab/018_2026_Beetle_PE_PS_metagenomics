...
# Get MAG read depth and convert to average depth per million reads
$MAG_names = 'mealworm_MAG_names.txt';
$output = 'mealworm_MAG_abundances_depth.txt';
system("tail -1 mealworm_coverage.txt | sed 's/\#rname/MAG_name/' | sed 's/\tendpos//' | sed 's/Mealworm_MAGs_combined_//g' > mealworm_MAG_abundances_depth.txt");
open($in, '<', $MAG_names);
while(<$in>) {
  chomp;
  $MAG = $_;
