...
for(n in 1:nrow(ancombc.standard.res)) { 
  if(ancombc.standard.res$diff_TreatmentPolyethylene[n] == TRUE & 
    ...
    taxa.diff_abn.PE <- rbind(taxa.diff_abn.PE, ancombc.standard.res$taxon[n])
  }
}
...
for(n in 1:nrow(ancombc.standard.res)) { 
  if(ancombc.standard.res$diff_TreatmentPolyethylene[n] == TRUE & 
     ...
     ancombc.starvation.res$diff_TreatmentPolyethylene[n] == TRUE & 
     ancombc.starvation.res$passed_ss_TreatmentPolyethylene[n] == TRUE) { 
    taxa.diff_abn.PS <- rbind(taxa.diff_abn.PS, ancombc.standard.res$taxon[n])
  }
}
...
for(n in 1:nrow(ancombc.standard.res)) { 
  if(ancombc.standard.res$diff_TreatmentPolyethylene[n] == TRUE & 
     ...
     ancombc.starvation.res$diff_TreatmentPolyethylene[n] == TRUE & 
     ancombc.starvation.res$passed_ss_TreatmentPolyethylene[n] == TRUE) { 
    taxa.diff_abn.species.PE <- rbind(taxa.diff_abn.species.PE, ancombc.standard.res$taxon[n])
  }
}
...
for(n in 1:nrow(ancombc.standard.res)) { 
  if(ancombc.standard.res$diff_TreatmentPolyethylene[n] == TRUE & 
     ...
     ancombc.starvation.res$diff_TreatmentPolyethylene[n] == TRUE & 
     ancombc.starvation.res$passed_ss_TreatmentPolyethylene[n] == TRUE) { 
    taxa.diff_abn.species.PS <- rbind(taxa.diff_abn.species.PS, ancombc.standard.res$taxon[n])
  }
}
...
```{r Load CFU data, results='hide', message=FALSE, warning=FALSE}
...
```{r CFU data 1, message=FALSE, warning=FALSE}
...
```{r CFU data 2, message=FALSE, warning=FALSE}
...
There is a clear outlier in the PS diet samples, so remove it and rerun the t-test in case it has an impact

```{r CFU data 3, message=FALSE, warning=FALSE}
...
Given there is no effect per diet, check if combining the data gives more power to detect an effect

```{r CFU data 4, message=FALSE, warning=FALSE}
...
Still not statistically significant; confirm this is true with the outlier removed

```{r CFU data 5, message=FALSE, warning=FALSE}
...
Based on these results, while there is a massive difference in bacterial versus fungal CFU, there is no statistically significant effect of the centrifugation step.
